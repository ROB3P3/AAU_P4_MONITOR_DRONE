    ...
    public:
        ViconClient(): Node("my_vicon_client") 
        {
            ...
            while (clientIsConnected.Connected)
            {
                ...
                RCLCPP_INFO(this->get_logger(), "Number of subjects: %d", viconSubjectCount.SubjectCount);
                if(viconSubjectCount.Result == ViconDataStreamSDK::CPP::Result::Success)
                {
                    for (unsigned int SubjectIndex = 0; SubjectIndex < viconSubjectCount.SubjectCount; ++SubjectIndex)
                    {
                        ...
                        {
                            RCLCPP_INFO(this->get_logger(), "Subject %d: %s", SubjectIndex, subjectName.SubjectName);
                            ...
                            {
                                RCLCPP_INFO(this->get_logger(), "Segment count: %d", segmentCount.SegmentCount);
