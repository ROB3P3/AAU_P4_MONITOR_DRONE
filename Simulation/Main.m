...
for k = testStart:testSlut
    ...
    meanX = (sum(abs(out.xPathError.Data(100:end-100)))*100)/length(out.xPathError.Data(100:end-100));
    meanY = (sum(abs(out.yPathError.Data(100:end-100)))*100)/length(out.yPathError.Data(100:end-100));
    ...
    meanZ = (sum(abs(out.zPathError.Data(100:end-100)))*100)/length(out.zPathError.Data(100:end-100));
    maxZ = max((abs(out.zPathError.Data(100:end-100))))*100;
    meanXYZ = sqrt(meanX^2+meanY^2+meanZ^2);
    % XY max error
    XYZLengthList = [];
    for n = 150:length(out.dxPathError.Data(150:end-100,1))
    ...
    meanDXerror = (sum(abs(out.dxPathError.Data(150:end-100,1)))*100)/length(out.dxPathError.Data(150:end-100,1));
    meanDYerror = (sum(abs(out.dyPathError.Data(150:end-100,1)))*100)/length(out.dyPathError.Data(150:end-100,1));
    meanDXYerror = sqrt(meanDXerror^2+meanDYerror^2);
    
    meanDX = (sum(abs(out.dxPath.Data(150:end-100,1)))*100)/length(out.dxPath.Data(150:end-100,1));
    meanDY = (sum(abs(out.dyPath.Data(150:end-100,1)))*100)/length(out.dyPath.Data(150:end-100,1));
    meanDXY = sqrt(meanDX^2+meanDY^2);
    % dXY max error
    dXYLengthList = [];
    dXYlist = [];
    refdXYlist = [];
    for n = 150:numel(out.dxPathError.Data)-150
        DX = abs(out.dxPathError.Data(n))*100;
        DY = abs(out.dyPathError.Data(n))*100;
        DXY = sqrt(DX^2+DY^2);
        dXYLengthList = [dXYLengthList, DXY];

        mDX = abs(out.dxPath.Data(n,1))*100;
        mDY = abs(out.dyPath.Data(n,1))*100;
        mDXY = sqrt(mDX^2+mDY^2);
        dXYlist = [dXYlist; mDXY];

        refDX = abs(out.dxPathRef.Data(n,1))*100;
        refDY = abs(out.dyPathRef.Data(n,1))*100;
        refDXY = sqrt(refDX^2+refDY^2);
        refdXYlist = [refdXYlist; refDXY];
    end
    maxDXY = max(dXYLengthList);

    

    %maxDX = max(abs(out.dxPathError.Data))*100
    %maxDY = max(abs(out.dyPathError.Data))*100
    %% Calculate Overshoot
    ...
    %% export
    meanErrors = [double(test), meanXY, meanXYZ, meanZ, maxZ, maxXY, maxXYZ, meanDXYerror, maxDXY, meanDXY];
    meanErrors = array2table(meanErrors, 'VariableNames', {'Test', 'Mean XY Error','Mean XYZ Error', 'Mean Z Error', 'Max Z Error', 'Max XY Error','Max XYZ Error', 'Mean dXY Error', 'Max dXY Error', 'Mean dXY'});
    ...
    if isfile('DATAdir\Test Results\Test Results.csv')
        ...
        summary = array2table(summary, 'VariableNames', {'Test', 'Mean XY Error','Mean XYZ Error', 'Mean Z Error', 'Max Z Error', 'Max XY Error','Max XYZ Error', 'Mean dXY Error', 'Max dXY Error', 'Mean dXY'});
