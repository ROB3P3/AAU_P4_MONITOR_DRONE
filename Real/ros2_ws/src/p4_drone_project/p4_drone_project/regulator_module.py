...
class RegulatorListener(Node):
    def __init__(self):
        super().__init__("regulator_node")
        self.viconSubscriber_ = self.create_subscription(Float64MultiArray, "/vicon_topic", self.onViconMsg, 10)
        self.pathPlannerSubscriber_ = self.create_subscription(PathPlannerMessage, "/pathplanner_topic", self.onPathPlannerMsg, 10)
        self.regulatorPublisher_ = self.create_publisher(RegulatedVelocity, "/command_topic", 10)
